...
SaravTV is a cross-platform IPTV player application built with Angular and Electron, forked from IPTVnator under MIT and supporting M3U/M3U8 playlists, Xtream Codes API, and Stalker portals.
...
- If DevTools is open, `agent-browser --cdp 9222 ...` may attach to the DevTools page instead of the SaravTV window (symptoms: `tab list` shows `about:blank`, empty snapshots, black screenshots). Inspect targets with `curl http://127.0.0.1:9222/json/list` and connect directly to the app page's `webSocketDebuggerUrl`.
...
- Dashboard Your Genre Picks use the pure `@iptvnator/recommendations/util`
  ...
  Up to 16 distinct seeds are selected round-robin across recent activity,
  favorites and imported history so one source cannot starve another; the four
  strongest genres produce catalog-backed rails of up to 20 cards. The ranker
  and balanced seed selector have no Angular, TMDB, storage or Electron
  dependencies.
